...
app.use(cors({
  origin: ['http://localhost:3003', 'http://localhost:5008'],
  credentials: true
}));
...
if (process.env.NODE_ENV === 'production') {
  ...
  app.get('/', (req, res) => {
    res.send('Сервер працює в режимі розробки. Використовуйте React додаток на порту 3003.');
  });
}
...
app.listen(PORT, () => {
  ...
  console.log(`Для доступу до React додатку використовуйте http://localhost:3003`);
});